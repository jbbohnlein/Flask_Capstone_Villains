...
#     # a_user = current_user_token.token
#     books = ShelvedBook.query.all()
    
#     # .filter_by(user_token = a_user).all()
#     response = books_schema.dump(books)
#     return jsonify(response)
...
#     book = ShelvedBook(key, title, author, year, avg_rating)  # Contact() comes from models.py. Looks similar but the ID will get written for us
